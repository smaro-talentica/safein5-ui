...
- **Location** (**`/location`**, worker) — page (`Location`, `src/pages/worker/Location/`) reachable from the bottom navigation bar. A client-only test harness for the **Geolocation API** — no backend or network call involved. **Get my location** requests a single high-accuracy fix via `navigator.geolocation.getCurrentPosition()` (15 s timeout, `maximumAge: 0` so a cached fix is never reused) and shows latitude, longitude, accuracy, and the time of the fix in the same diagnostic-panel style the Alert page uses. It deliberately uses a **one-shot fix per tap rather than `watchPosition()`** — there is no background tracking and no continuous GPS drain. The three real failure modes are reported distinctly with an actionable message rather than a raw error code: **permission denied** (the common one — note that browsers remember a denial per-origin, so re-testing it means clearing the site's location permission in browser settings), **position unavailable**, and **timeout**. Like the camera, geolocation is gated behind a **secure context** — over plain HTTP the API is absent, and the page shows an "isn't available" message instead of a non-functional button (see the HTTPS/secure-context section below). Indoor fixes are often coarse (±1 km or worse is normal when the device falls back to wifi/cell positioning rather than GPS) — that is expected, not a bug.
- **Alert** (**`/alert`**, worker) — page (`Alert`) reachable from the bottom navigation bar. A client-only test harness for browser/OS notifications — no backend or network call involved. **Start** requests `Notification` permission (if not already granted) and, once granted, fires a repeating local notification via the service worker's `showNotification()` at a random interval between 5–15 seconds until **Stop** is pressed; each notification uses a unique tag so it's always shown as new rather than silently replacing the previous one. The page also shows whether it's currently running as an **installed PWA** or a plain **browser tab** (via the same standalone-display-mode detection the install prompt uses), which is useful for confirming whether "Add to Home Screen" actually produced a true standalone install on a given browser. A live on-page diagnostic panel shows permission state, running status, fire count, and the last-fired time. Tapping a fired notification focuses an existing app window or opens one at `/` (via a `notificationclick` handler in `src/sw.ts`) — intentionally not deep-linked to a specific route, since iOS ignores the target path passed to `clients.openWindow()`. On iOS, notifications require the app to be installed to the home screen (iOS ≥ 16.4); until then the page shows "Add to Home Screen" instructions instead of a non-functional Start button. This is inherently foreground-only — the repeat loop is a page-side timer that stalls if the app is backgrounded or closed, on every platform. **Known gap — Opera for Android:** `Notification.requestPermission()` resolves straight to `denied` with no prompt ever shown, reproduced in both a plain browser tab and an installed PWA, even with Opera's own site-level notification setting and the OS-level toggle both enabled; no separate Android app entry is created for the installed PWA (Opera does not appear to install a true WebAPK, so there's no distinct OS notification channel to grant). Consistent with Opera for Android's already-documented weaker notification/PWA support elsewhere in this doc (see the install-prompt notes below) — treated as a platform limitation, not an app bug.
...
The QR scanner needs the **device camera**, the Location page needs **geolocation**, and installable-PWA features like **service workers** (offline precache, the Alert page's notifications) need a **service worker registration** — all are only available in a **secure context**: HTTPS with a certificate the device *trusts*, or `localhost`. When that context isn't trusted, the failure modes look very different but share the same root cause: the camera silently fails (black preview, no error), while a service worker **fails to register at all, with no visible error anywhere** — the page still loads and navigates normally over plain HTTPS-with-warnings, which makes this easy to mistake for an app bug rather than a missing cert trust step. If `navigator.serviceWorker.getRegistrations()` ever comes back empty on a device where the app otherwise seems to work, suspect this first. To test camera/PWA features on a real phone:
...
3. Grant the phone a **secure context** using one of:
   - **Trust the mkcert CA (recommended, permanent):** this project generates its dev certificate via `vite-plugin-mkcert`, which keeps its **own** CA — separate from any system-wide `mkcert` install — at `%USERPROFILE%\.vite-plugin-mkcert\rootCA.pem` on Windows (macOS/Linux: `~/.vite-plugin-mkcert/rootCA.pem`). This directory (and the CA inside it) is only created the first time `npm run dev` actually runs. Transfer `rootCA.pem` to the phone and install it as a trusted **CA certificate** (Android: Settings → Security → Install a certificate → CA certificate; iOS: install the profile, then enable it under Settings → General → About → Certificate Trust Settings). The origin is then fully trusted — no warnings, and service workers can register. After installing, fully close (don't just background) any already-open tab/installed PWA for this origin before retesting, so it doesn't reuse an earlier "untrusted" verdict.
   - **Chrome insecure-origin flag (quick, per-device):** on the phone's Chrome open `chrome://flags/#unsafely-treat-insecure-origin-as-secure`, set it **Enabled**, add `https://<your-lan-ip>:5173`, and relaunch. This forces a secure context for the **page** without installing a cert — note this flag is Chrome-specific and may not extend the same trust to an installed/standalone PWA window, so the CA-install method above is more reliable for testing service-worker-dependent features in an installed PWA.
...
- **Custom service worker** (`src/sw.ts`, `injectManifest` strategy) — Workbox app-shell precaching, plus a `notificationclick` handler (focuses an existing app window or opens one at `/`; used by the Alert page's test notifications, see above).
- Web app manifest (standalone display, theme/background colors, three icon entries): `public/pwa-192.png` (`purpose: "any"`, 192×192) and `public/pwa-512.png` (`purpose: "any"`, 512×512) plus `public/pwa-maskable.png` (`purpose: "maskable"`, 1024×1024) — the logo is scaled to ~72% and centered on a canvas filled with the icon's own background color, keeping it inside Android's safe-zone so platform icon masks (circle, squircle, etc.) crop only background, not the logo, fixing the inconsistent cropping previously seen across Firefox/Opera/Edge. **Every icon entry must declare its own real pixel dimensions.** A single PNG listed as `sizes: "192x192 512x512"` (a form only valid for multi-resolution `.ico` files) is rejected by Chrome when the decoded bitmap doesn't match either declared size — that failed the "valid icon ≥192×192" installability criterion and silently downgraded the install to a browser shortcut rather than a true standalone WebAPK. The two `any` icons are resized from the full-resolution source `public/pwa.png` (1966×1966, kept only as that source and not referenced by the manifest) using `sharp`; regenerate them if the logo changes.
- Offline asset precaching via Workbox (`js`, `css`, `html`, `ico`, `png`, `svg`, `woff2`)
- **Custom install prompt** — an in-app banner (`InstallPrompt`, `src/components/feature/InstallPrompt/`, driven by the `useInstallPrompt` hook) offers "Add to Home Screen" until the app is installed, including on the logged-out `/login` screen. **Mobile only** (gated by `useIsMobile`) — desktop browsers never show it, regardless of engine. It is hidden while the app is running as the installed app itself (`display-mode: standalone`), after the user dismisses it (✕ — remembered permanently via `localStorage`, `src/components/feature/InstallPrompt/helper.tsx`), and whenever there is nothing actionable to show — it does not show a button-less generic message just to be present. Install detection is app-wide: an `InstallPromptProvider` mounted at the app root captures the one-shot `beforeinstallprompt` event regardless of the current route, so it is not missed on the entry screen, and classifies the current browser into one of six buckets: **Android Chrome/Samsung Internet/Edge** (`chromium-standard`) reliably fire `beforeinstallprompt`, so the banner shows the real native install button via that captured event; **Android Opera** (`opera-android`) is Chromium-based and PWA-capable but is known to fire `beforeinstallprompt` unreliably (a documented gap in that browser specifically, not a platform limitation), so it always shows an Install button optimistically — clicking it uses the real captured event if one exists, and falls back **in place** to manual menu (⋮) → Add to Home screen instructions if none ever fired; **Android Firefox** (`firefox-android-unsupported`) exposes no install path at all, so the banner shows a plain "No PWA Support" message with no button and no instructions; **iOS Safari** (`ios-safari`) shows manual Share → Add to Home Screen steps; **any other iOS browser** (`ios-other-webkit` — Chrome/Edge/Opera/Firefox for iOS, detected via their `CriOS`/`EdgiOS`/`OPT`/`FxiOS` UA tokens) is forced onto WebKit by Apple but, unlike Safari, has no "Add to Home Screen" option in its share sheet, so it shows a message telling the user to open the site in Safari to install instead. Note: the native prompt only becomes available after the app is installable **and** the user has engaged with the page — never on the very first paint; once Chrome has already installed the app for a given profile, it will not re-offer `beforeinstallprompt` in a normal tab, so the `chromium-standard` banner stays hidden there rather than showing with no button.
...
- **Display-mode badge** — a small, always-visible **PWA** / **BRO** label fixed to the top-right corner of every screen (`RootLayout`, `src/AppRoute/index.tsx`), reusing the same `installed`/standalone-display-mode detection as the install prompt above. Useful for confirming, on-device, whether "Add to Home Screen" actually produced a true standalone install (`PWA`) versus just a shortcut that still opens in the browser (`BRO`) — some browsers (e.g. Opera, Edge for Android) have been observed not producing a true standalone install even after the add-to-home-screen flow.
