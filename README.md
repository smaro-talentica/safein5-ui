...
- Web app manifest (standalone display, theme/background colors, single 192/512 PNG icon — `public/pwa.png`, declared with `sizes: "192x192 512x512"` since it's one source image used at both sizes). No `maskable` icon is declared yet — the source icon has no safe-zone padding, so browsers were aggressively cropping it into inconsistent shapes across Firefox/Opera/Edge; a proper padded maskable variant can be added later.
